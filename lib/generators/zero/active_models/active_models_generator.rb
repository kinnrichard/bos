    ...
    class ActiveModelsGenerator < Rails::Generators::Base
      ...
      def generate_data_interface(table, class_name, relationships = {})
        ...
        <<~TYPESCRIPT
           ...
           */
          export type Create#{class_name}Data = Omit<#{class_name}Data, 'id' | 'created_at' | 'updated_at'#{relationship_exclusions}>;

          /**
           * Update #{class_name} data interface
           * All fields optional except id, excludes relationships
           */
          export type Update#{class_name}Data = Partial<Omit<#{class_name}Data, 'id' | 'created_at' | 'updated_at'#{relationship_exclusions}>>;
        TYPESCRIPT
      end
