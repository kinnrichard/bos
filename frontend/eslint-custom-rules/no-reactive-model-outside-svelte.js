...
export default {
  meta: {
    ...
    messages: {
      reactiveModelOutsideSvelte: 'ReactiveModel/ReactiveRecord should only be used in Svelte files (.svelte). Use ActiveModel/ActiveRecord for non-Svelte contexts.',
      reactiveModelInTest: 'ReactiveModel/ReactiveRecord in test files may cause issues. Consider using ActiveModel for more predictable testing.',
      activeModelInSvelte: 'ActiveModel/ActiveRecord in Svelte files will not be reactive. Use ReactiveModel/ReactiveRecord for automatic UI updates.',
      inconsistentNaming: 'Inconsistent model naming. Use Reactive* for Svelte files, Active* for other contexts.',
  ...
  create(context) {
    ...
    function isReactiveModelUsage(node) {
      if (node.type === 'Identifier') {
        return REACTIVE_MODEL_PATTERNS.some(pattern => 
          node.name.includes(pattern)
        );
      }
      if (node.type === 'ImportSpecifier' || node.type === 'ImportDefaultSpecifier') {
        return REACTIVE_MODEL_PATTERNS.some(pattern => 
          node.imported?.name?.includes(pattern) || node.local?.name?.includes(pattern)
        );
      }
      return false;
    }
    ...
    function isActiveModelUsage(node) {
      if (node.type === 'Identifier') {
        return ACTIVE_MODEL_PATTERNS.some(pattern => 
          node.name.includes(pattern)
        );
      }
      if (node.type === 'ImportSpecifier' || node.type === 'ImportDefaultSpecifier') {
