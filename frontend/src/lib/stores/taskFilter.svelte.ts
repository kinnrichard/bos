...
export const taskStatusFilter = createFilterStore({
  ...
  defaultSelected: taskStatusOptions.map(opt => opt.value), // All selected by default
  supportDeleted: true,
...
export function shouldShowTask(task: { title?: string; description?: string; status?: string; discarded_at?: string | null }): boolean {
  // Apply search filter
...
export const taskFilter = {
  get selectedStatuses() { return taskStatusFilter.selected; },
  get showDeleted() { return taskStatusFilter.showDeleted; },
  get searchQuery() { return taskSearch.searchQuery; },
  get searchFields() { return taskSearch.searchFields; },
  set selectedStatuses(value: string[]) { taskStatusFilter.setSelected(value); },
  set showDeleted(value: boolean) { taskStatusFilter.setShowDeleted(value); },
  set searchQuery(value: string) { taskSearch.searchQuery = value; },
  set searchFields(value: ('title' | 'description')[]) { taskSearch.searchFields = value; }
};

// Legacy compatibility function (to be removed after migration)
export function shouldShowTaskLegacy(task: { discarded_at?: string | null; status?: string }, statuses: string[], showDeleted: boolean = false): boolean {
...
export function getTaskFilterFunction() {
  return (task: { title?: string; description?: string; status?: string; discarded_at?: string | null }) => shouldShowTask(task);
}
