...
export async function createJob(data: CreateJobData): Promise<JobMutationResult> {
  ...
  if (!data.status?.trim()) throw new Error('Status is required');
  if (!data.priority?.trim()) throw new Error('Priority is required');
 ...
 */
async updateStatus(status: 'open' | 'in_progress' | 'paused' | 'waiting_for_customer' | 'waiting_for_scheduled_appointment' | 'successfully_completed' | 'cancelled'): Promise<{ id: string }> {
