...
export const Client = {
  ...
  /**
   * Get all clients (includes discarded records like Rails Task.all)
   * @returns Zero query result with array of clients
   * 
   * @example
   * ```typescript
   * const allClients = Client.all();
   * console.log(allClients.current); // Array of clients including discarded
   * ```
   */
  all() {
    const zero = getZero();
  ...
  /**
   * Find clients matching conditions (includes discarded)
