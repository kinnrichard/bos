...
export async function upsertUser(data: (CreateUserData & { id?: string }) | (UpdateUserData & { id: string })): Promise<UserMutationResult> {
  ...
  try {
    ...
    await zero.mutate.users.upsert(upsertData);
...
// ActiveRecord-style instance class for individual user records

/**
 * ActiveRecord-style instance class for User
 * Provides Rails-compatible instance methods: update(), delete(), restore()
 * 
 * Generated from Rails model: User
 * 
 * @example
 * ```typescript
 * const user = User.find('123').current;
 * if (user) {
 *   const instance = new UserInstance(user);
...
export class UserInstance extends RecordInstance<User> {
   ...
   */
  inspect(): string {
    return `#<UserInstance id: ${this.data.id}, name: "${(this.data as any).name}">`;
  }
}

/**
 * Factory function to create UserInstance from data
 * Used internally by ReactiveRecord and ActiveRecord
 */
export function createUserInstance(data: User): UserInstance {
...
export const User = {
  ...
  /**
   * Get all users (includes discarded records like Rails Task.all)
   * @returns Zero query result with array of users
   * 
   * @example
   * ```typescript
   * const allUsers = User.all();
   * console.log(allUsers.current); // Array of users including discarded
   * ```
   */
  all() {
    const zero = getZero();
  ...
  /**
   * Find users matching conditions (includes discarded)
