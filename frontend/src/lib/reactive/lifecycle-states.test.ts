...
import { ReactiveCoordinator, type CoordinatorState, type VisualState } from '../../src/lib/reactive/coordinator';
import {
  MockReactiveQuery,
  MockCoordinator,
  TestDataFactory,
    ...
    it('should maintain state consistency across all transitions', () => {
      ...
      states.forEach((state, index) => {
        ...
        if (state.shouldShowEmpty) {
          ...
          expect(state.displayData === null || (Array.isArray(state.displayData) && state.displayData.length === 0)).toBe(true);
        }
      });
    });
