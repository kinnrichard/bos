...
test.describe('Drag-Drop Feature Preservation', () => {
  ...
  createPageTest(
    ...
    async (page, { factory, cleanup }) => {
      ...
      try {
        ...
        await page.locator(`[data-task-id="${task1Id}"]`).dragTo(page.locator(`[data-task-id="${task3Id}"]`));
  ...
  createPageTest(
    ...
    async (page, { factory, cleanup }) => {
      ...
      try {
        // Change task status
        const statusButton = page.locator(`[data-task-id="${task1Id}"] .status-emoji, [data-task-id="${task1Id}"] .task-status`).first();
        if (await statusButton.count() > 0) {
          ...
          const statusOption = page.locator('.status-dropdown .status-option[data-status="in_progress"], .status-option').first();
  ...
  createPageTest(
    ...
    async (page, { factory, cleanup }) => {
      ...
      try {
        ...
        const newTaskButton = page.locator('.new-task-button, .new-task-row button, button:has-text("New"), button:has-text("Add")');
        ...
        // Enter task title
        const taskInput = page.locator('.new-task-row input, input[placeholder*="task"], .task-input').first();
        if (await taskInput.count() > 0) {
          ...
          const newTaskElement = page.locator('.task-title:has-text("New Test Task"), [data-task-id]:has-text("New Test Task")');
          if (await newTaskElement.count() > 0) {
            await expect(newTaskElement.first()).toBeVisible();
            
            // Try to get the task ID for drag testing
            const parentTaskElement = newTaskElement.locator('xpath=ancestor-or-self::*[@data-task-id]').first();
  ...
  createPageTest(
    ...
    async (page, { factory, cleanup }) => {
      ...
      try {
        ...
        const confirmButton = page.locator('.deletion-modal button:has-text("Delete"), button:has-text("Confirm"), .modal button:has-text("Delete")');
  ...
  createPageTest(
    ...
    async (page, { factory, cleanup }) => {
      ...
      try {
        await dragDropHelper.reorderTask(
          additionalTasks[0].id,
          task1Id!,
          'before'
        );
