...
test.describe('Client Job Creation Feature', () => {
  ...
  test.describe('Route Navigation Tests', () => {
    ...
    test('should provide "Create First Job" button in empty state', async ({ page }) => {
      ...
      const createFirstJobButton = page.locator('a[href="/clients/' + client.id + '/jobs/new"]', {
  ...
  test.describe('Client Validation Tests', () => {
    ...
    test('should show loading state while fetching client data', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="job-card-skeleton"], .loading-skeleton')).toBeVisible();
  ...
  test.describe('Job Creation Flow Tests', () => {
    test('should auto-focus on job title field in creation mode', async ({ page }) => {
      ...
      const editableTitle = page.locator('.job-title input, .job-title textarea, [contenteditable="true"]');
    ...
    test('should validate job title and show error for empty submission', async ({ page }) => {
      ...
      await expect(page.locator('.toast-error, [data-testid="toast-error"]')).toBeVisible({ timeout: 5000 });
    ...
    test('should successfully create job and navigate to job page', async ({ page }) => {
      ...
      const titleElement = page.locator('.job-title input, .job-title [contenteditable="true"]').first();
      await expect(titleElement).toBeVisible();
  ...
  test.describe('UI State Tests', () => {
    test('should disable toolbar buttons in creation mode', async ({ page }) => {
      ...
      const technicianButton = page.locator('[data-testid="technician-assignment-button"], .technician-assignment-button');
  ...
  test.describe('Integration Tests', () => {
    test('should complete end-to-end workflow from client page to job creation to job view', async ({ page }) => {
      const timestamp = Date.now();
      const client = await dataFactory.createClient({ 
      ...
      // Click new job button
      await page.click('a[href="/clients/' + client.id + '/jobs/new"]');
      await expect(page).toHaveURL(`/clients/${client.id}/jobs/new`);
    ...
    test('should preserve existing job view behavior after implementing creation feature', async ({ page }) => {
