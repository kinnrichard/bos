...
import { Epic008Migration, validateMigration } from '../src/lib/models/migration/epic-008-migration';
import * as fs from 'fs/promises';
import * as path from 'path';

test.describe('Epic-008 Rails Generator Validation', () => {
  const projectRoot = '/Users/claude/code/bos/frontend';
  let migration: Epic008Migration;

  test.beforeEach(() => {
    migration = new Epic008Migration(projectRoot);
  });
  ...
  test.describe('Generated CRUD Operations', () => {
    ...
    test('should have proper function signatures with validation', async () => {
      ...
      try {
        ...
        expect(content).toContain('[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}');
  ...
  test.describe('Migration Compatibility', () => {
    test('should run migration validation successfully', async () => {
      ...
      if (!validation.success) {
        ...
        const hasExpectedMissingFiles = validation.errors.some(error => 
  ...
  test.describe('Error Handling and Validation', () => {
    test('should have comprehensive error messages', async () => {
      ...
      try {
        ...
        expect(content).toContain('Zero client not initialized. Please ensure Zero is properly set up.');
    ...
    test('should validate input parameters properly', async () => {
      ...
      try {
        ...
        expect(content).toContain('if (data.lock_version === undefined || data.lock_version === null)');
        expect(content).toContain('if (data.applies_to_all_targets === undefined || data.applies_to_all_targets === null)');
        
        // Should validate ID format with UUID regex
        expect(content).toContain('if (!id.match(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i))');
