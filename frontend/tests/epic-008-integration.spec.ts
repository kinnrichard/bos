...
import { Task, createTask, updateTask, discardTask } from '../src/lib/zero/task.generated';
import { ActiveRecord } from '../src/lib/models/base/active-record';
import type { BaseRecord } from '../src/lib/models/base/types';
  ...
  query: {
    tasks: {
      where: (field: string, value: any) => {
        ...
        return {
          ...
          orderBy: (orderField: string, direction: string) => ({
            ...
            many: async () => [...mockDatabase].sort((a, b) => {
              if (direction === 'desc') {
      ...
      orderBy: (field: string, direction: string) => ({
        where: (f: string, v: any) => ({
          many: async () => mockDatabase.filter(item => {
        ...
        materialize: () => ({
          data: Promise.resolve([...mockDatabase].sort((a, b) => {
            if (direction === 'desc') {
              return b[field] - a[field];
            }
            return a[field] - b[field];
...
(global as any).getZero = () => integrationMockZero;
...
test.describe('Epic-008 Integration Tests', () => {
  ...
  test.describe('End-to-End CRUD Operations', () => {
    test('should create, read, update, and delete tasks', async () => {
      ...
      // Update the task
      const updateResult = await updateTask('test-task-1', {
      ...
      const discardResult = await discardTask('test-task-1');
  ...
  test.describe('Reactive vs Non-Reactive Comparison', () => {
    ...
    test('should handle updates in both interfaces', async () => {
      ...
      await updateTask('test-task-1', { title: 'Updated Title' });
  ...
  test.describe('Discard Gem Integration', () => {
    ...
    test('should handle undiscard operations', async () => {
      ...
      // Undiscard the task
      await undiscardTask('test-task-1');
  ...
  test.describe('Migration Compatibility', () => {
    test('should work with existing data patterns', async () => {
      ...
      await updateTask('legacy-task-1', { title: 'Updated Legacy Task' });
